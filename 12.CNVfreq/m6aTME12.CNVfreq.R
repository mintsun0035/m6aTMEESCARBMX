...
setwd("C:\\Users\\sun\\Desktop\\m6a-esca\\12.CNVfreq")      #工作目录
rt=read.table(inputFile, header=T, sep="\t", check.names=F, row.names=1)    #读取输入文件矩阵
GAIN=rowSums(rt> 0)       #?????????ӵ???Ʒ??Ŀ
LOSS=rowSums(rt< 0)       #??????ȱʧ????Ʒ??Ŀ
GAIN=GAIN/ncol(rt)*100      #?????????ӵİٷ???
LOSS=LOSS/ncol(rt)*100      #??????ȱʧ?İٷ???
